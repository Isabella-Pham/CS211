...
first_weight = {"testA.txt":5.0, "testB.txt":10.0, "testC.txt":5.0, "testD.txt":5.0, "testE.txt":5.0, "testF.txt":5.0, "testG.txt":5.0, "testH.txt":5.0, "testI.txt":5.0}
    ...
    # Answer says not no-solution and test says not no-solution
        ...
        for i in range(0, 16) :
            if len(test_board[i]) != 16 :
                print "Expected 16 columns. Observed %d columns in row %d" % (len(test_board[i]), i+1)
                return False
                ...
                for j in range(0, 16) :
                    ...
                    for c in range(0, 16):
                        if c!=j and test_board[i][j]==test_board[i][c]:
                            print "Duplicated in row %d, col %d and col %d " % (i, c, j)
                            return False

                    #check vertivally
                    for r in range(0,16):
                        if r!=i and test_board[i][j]==test_board[r][j]:
                            print "Duplicated in col %d, row %d and row %d " % (j, i, r)
...
#            	print "Deleting the directory obj_temp."
    ...
    total_score = final_release_scores['first'] + final_release_scores['second'] + final_release_scores['third']
